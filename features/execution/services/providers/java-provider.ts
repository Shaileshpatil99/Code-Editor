...
import { detectJavaMainClass } from "../../lib/java-detector";

export class JavaExecutionProvider implements IExecutionProvider {
  readonly id = "java";
  readonly name = "Java Local Engine (JDK 24)";
  ...
  async run(
    ...
    // Deterministically detect main class
    const detection = detectJavaMainClass(context.files, context.activeFile);

    if (!detection.mainClass) {
      onEvent({
        type: "stderr",
        data: `\r\n\x1b[31mError: ${
          detection.error ||
          "No runnable main method found. Please define 'public static void main(String[] args)'."
        }\x1b[0m\r\n`,
      });
      onEvent({ type: "status", status: "COMPILE_ERROR", exitCode: 1 });
  ...
  sendInput(data: string): void {
    ...
    fetch("/api/execute/stdin", {
      ...
      console.error("Failed to send stdin to Java process:", err);
    });
  }
  ...
  async stop(): Promise<void> {
    ...
    if (this.currentRunId) {
      ...
      try {
        ...
        console.error("Error stopping Java process:", err);
      }
    }
  }
}
